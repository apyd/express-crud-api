...
interface CheckoutModelInterface
  extends Model<
    InferAttributes<CheckoutModelInterface>,
    InferCreationAttributes<CheckoutModelInterface>
  > {
...
const CheckoutModel = sequelize.define<CheckoutModelInterface>(
  "Checkout",
  {
    id: {
      type: DataTypes.UUID,
    ...
    cartId: {
      type: DataTypes.UUID,
      allowNull: false,
      references: { model: CartModel, key: "id" },
    },
    comments: { type: DataTypes.STRING, allowNull: false },
    status: {
      type: DataTypes.STRING,
      validate: { isIn: [["created", "completed"]] },
    },
  },
  {
    tableName: "checkouts",
...
CheckoutModel.hasOne(DeliveryModel, {
